...
;Задача №7.Определите функцию, удаляющую из исходного списка элементы с четными номерами.
...
(defun compare (x lst) ;возвращает список из элементов не равных данному
(( lambda (f r)
    (cond
        ((null lst) nil)
        ((eq x f) (compare x r))
...
;Задача №18.Определите предикат, проверяющий, является ли аргумент одноуровневым списком.
...
;Задача №31.Определите функцию (ПЕРВЫЙ-СОВПАДАЮЩИЙ х у), которая возвращает первый элемент, входящий в оба списка х и у, в противном случае NIL.

(defun member1 (a x) ;сравнивает один элемент с элементами второго списка
    (cond
        ((null x) nil)
        ((eq a (car x)) T)
        (t (member1 a (cdr x)) )
...
;Задача №37.Определите функцию ПЕРЕСЕЧЕНИЕ, формирующую пересечение двух множеств, т.е. множество из их общих элементов.
...
;Задача №40.Определите функцию РАЗНОСТЬ, формирующую разность двух множеств, т.е. удаляющую из первого множества все общие со вторым множеством элементы.
...
;Задача №43.Определите функцию, подсчитывающую количество всех вершин данного дерева заданной высоты.
...
;Задача №48.Функция GET возвращает в качестве результата NIL в том случае, если у символа нет данного свойства, либо если значением этого свойства является NIL. Следовательно, функцией GET нельзя проверить, есть ли некоторое свойство в списке свойств. Напишите предикат (ИМЕЕТ-СВОЙСТВО символ свойство), который проверяет, обладает ли символ данным свойством
...
(setf (get 'духи 'фирма) 'Шанель) ;создание новой пары свойства
(setf (get 'духи 'название) 'Мадмуазель)
(setf (get 'духи 'производитель) 'Франция)
(setf (get 'духи 'аромат) 'цветочный)

(print (symbol-plist 'духи)) ; возвращает список, который содержит пары свойств для символа
